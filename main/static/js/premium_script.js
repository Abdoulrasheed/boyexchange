...
jQuery(function($){ 
 	...
 	function get_exchange_step1(id){
		...
		$.ajax({
			type: "POST",
			url: "https://atchange.net/premium_action-exchange_stepselect.html?meth=post&yid=e70e47401c88&ynd=0&lang=en",
			dataType: 'json',
			data: param,
			error: function(res, res2, res3){
						console.log('Error text, text1: ' + res2 + ',text2:' + res3);
			...
			if(nowdata >= second){
				if(redir == 0){
					...
					if(durl.length > 0){
						...
						$.ajax({
							type: "POST",
							url: "https://atchange.net/premium_action-refresh_status_bids.html?meth=post&yid=e70e47401c88&ynd=0&lang=en&auto_check=1",
							dataType: 'json',
							data: param,
							error: function(res, res2, res3){
										console.log('Error text, text1: ' + res2 + ',text2:' + res3);
...
jQuery(function($){
	...
	jQuery(function($){ 
		...
		$(document).on('click', '.captcha_reload', function(){
			...
			$.ajax({
				type: "POST",
				url: "https://atchange.net/premium_action-captcha_reload.html?meth=post&yid=e70e47401c88&ynd=0&lang=en",
				dataType: 'json',
				data: param,
				error: function(res,res2,res3){
							console.log('Error text, text1: ' + res2 + ',text2:' + res3);
	...
	if($('.napssms_block').length > 0){
		...
		$(document).on('click', '#napssms_reload', function(){
			if(!$(this).prop('disabled')){
				...
				$.ajax({
					type: "POST",
					url: "https://atchange.net/premium_action-resend_napssms_bids.html?meth=post&yid=e70e47401c88&ynd=0&lang=en",
					dataType: 'json',
					data: param,
					error: function(res, res2, res3){
								console.log('Error text, text1: ' + res2 + ',text2:' + res3);
		...
		$(document).on('click', '#napssms_send', function(){
			if(!$(this).prop('disabled')){
				...
				$.ajax({
					type: "POST",
					url: "https://atchange.net/premium_action-save_napssms_bids.html?meth=post&yid=e70e47401c88&ynd=0&lang=en",
					dataType: 'json',
					data: param,
					error: function(res, res2, res3){
								console.log('Error text, text1: ' + res2 + ',text2:' + res3);
...
jQuery(function($){ 
	...
	$(document).on('click', '.js_reserv', function(){
		$(document).JsWindow('show', {
			window_class: 'update_window',
			title: 'Request to reserve <span id="reserv_box_title"></span>"',
	...
	if($('#hexch_html').length > 0){
		$(document).on('click', '.js_exchange_link', function(){
			if(!$(this).hasClass('active')){
				...
				$.ajax({
					type: "POST",
					url: "https://atchange.net/premium_action-exchange_widget.html?meth=post&yid=e70e47401c88&ynd=0&lang=en",
					dataType: 'json',
					data: param,
					error: function(res, res2, res3){
								console.log('Error text, text1: ' + res2 + ',text2:' + res3);
...
jQuery(function($){
	...
	function go_exchange_calc(sum, dej){
        ...
        $.ajax({
            type: "POST",
            url: "https://atchange.net/premium_action-exchange_calculator.html?meth=post&yid=e70e47401c88&ynd=0&lang=en",
            data: param,
	        dataType: 'json',
 			error: function(res, res2, res3){
						console.log('Error text, text1: ' + res2 + ',text2:' + res3);
